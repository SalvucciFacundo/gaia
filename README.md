...
| Icon | Meaning |
|------|---------|
| ● connected | Server is online and tools are available |
...
GAIA gestiona el contexto de la conversaci�n en tres capas para balancear costo y fidelidad:

| Capa | Qu� hace | Default |
|------|----------|---------|
| **1. Mensajes recientes** | �ltimos N mensajes �ntegros (sin p�rdida) | ? Siempre activo |
| **2. Compactaci�n** | Resumen autom�tico de mensajes viejos v�a LLM | ? Siempre activo |
| **3. Knowledge Graph Recall** | Facts extra�dos autom�ticamente + b�squeda | ? Opt-in via /kg on |

Las capas 1 y 2 siempre est�n activas. La capa 3 es opt-in para sesiones largas donde quer�s ahorrar tokens.
...
Ver [docs/context-layers.md](docs/context-layers.md) para la explicaci�n completa.

### ?? Progressive Skill Loading

Los subagentes cargan skills **bajo demanda** seg�n su tarea, no todos al inicio:

1. El orquestador pasa un **cat�logo de skills disponibles** a cada subagente
2. El subagente **decide qu� skills cargar** seg�n su dominio y tarea
3. Solo los skills necesarios se cargan en contexto (~3k tokens de �ndice + skill specific)

**Auto-learning**: despu�s de 5 ejecuciones de un subagente, el sistema crea autom�ticamente un skill con los patrones observados:

```bash
# Se crea autom�ticamente en ~/.gaia/skills/:
#   explorer-patterns/    � patrones de exploraci�n
#   implementer-patterns/ � patrones de implementaci�n
#   verifier-patterns/    � patrones de verificaci�n

# El usuario puede editarlos para refinar los patrones:
gaia skills list          # Ver skills instalados
```

El Learner subagente tambi�n analiza el c�digo y propone skills nuevos v�a `@learner analyze`.
