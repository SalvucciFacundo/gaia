# Delta for Async Delegation + Direct Subagent Chat
...
The system MUST route user input from TUI → `Brain.ProcessMessage()` → LLM call → response → TUI display. The Brain SHALL orchestrate the full turn including tool-call loops. The Brain MUST also detect `@<name>` prefixes and route directly to the named subagent via SpawnAsync, bypassing the normal orchestrator fan-out.

(Previously: Brain.ProcessMessage handled all user input synchronously with no direct subagent routing.)
...
#### Scenario: Direct subagent message

- GIVEN the user types "@explorer find auth bugs"
- WHEN the message is submitted
- THEN Brain parses the @explorer prefix and routes via SpawnAsync
- AND the TUI remains responsive while the subagent runs

### Requirement: Tool Registry

The system MUST maintain a registry mapping tool names to implementations. The Brain SHALL look up tools by name when the LLM issues a tool_call. The registry MUST support per-task tool filtering — when a subagent has a declared AllowedTools set, only those tools SHALL be available during that task's execution. Tool execution MUST respect the task's cancellation context; if the task is cancelled, in-flight tool executions MUST be interrupted.

(Previously: Tool registry had no per-task filtering or cancellation-aware execution.)
...
#### Scenario: Filtered tool access

- GIVEN a subagent with AllowedTools: ["file_read"]
- WHEN the subagent calls "shell_exec"
- THEN the tool call is rejected as outside the allowed set

#### Scenario: Tool cancelled mid-execution

- GIVEN a tool is executing within a task
- WHEN the task's context is cancelled
- THEN the tool execution is interrupted and returns a cancellation error
