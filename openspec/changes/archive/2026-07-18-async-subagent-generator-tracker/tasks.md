...
- [x] 3.1 Create `internal/adapters/db/subagent_defs.go` — DefRepository CRUD against SQLite subagent_defs table, JSON serialization for AllowedTools/Skills, validate tool names against ToolRegistry
- [x] 3.2 Create `internal/agent/dynamic.go` — DynamicSubagent (implements Subagent interface), DynamicSubagentFactory, system prompt assembly from def.SystemPrompt + def.Personality, AllowedTools enforcement on Execute
- [x] 3.3 Create `internal/adapters/tui/interview.go` — interviewModel with step enum (name→desc→tools→skills→personality→confirm), back-navigation, multi-select for tools, input validation
- [x] 3.4 Wire `/create-agent` command — TUI detects /create-agent, starts interviewModel, on confirm calls DynamicLoader.CreateFromDef (persist + register + namespace)
- [x] 3.5 Add startup loader in main.go — after SQLite init, DynamicLoader.LoadAll() → registers each persisted def with Registry
- [x] 3.6 Write/tests: dynamic_test.go — DynamicSubagent interface tests, DynamicLoader with mock DefRepository, tool validation at CreateDef time, factory closure, LoadAll startup reload, RemoveDynamic

## Phase 4: Upstream Tracker (~500-600 lines)

- [x] 4.1 Create `internal/adapters/db/tracker.go` — tracker_releases + tracker_state CRUD (etag, last_checked tag, release cache)
- [x] 4.2 Create `internal/tracker/monitor.go` — GitHubReleaseMonitor with http.Client, ETag caching (If-None-Match), GITHUB_TOKEN auth, CheckLatest/ListReleases methods
- [x] 4.3 Create `internal/tracker/analyzer.go` — ChangelogAnalyzer with regex-based conventional-commit classification (feat/fix/breaking_change/refactor/docs/other), AnalyzeRelease/DiffReleases methods
- [x] 4.4 Create `internal/tracker/manifest.go` — PortManifest (Load/Save with atomic write via temp file + os.Rename)
- [x] 4.5 Create `internal/tracker/report.go` — markdown report generator (grouped by ManifestStatus table)
- [x] 4.6 Add `gaia tracker {check,report,port}` CLI dispatch to `cmd/gaia/main.go` — check calls CheckLatest + DiffWithManifest, report prints markdown, port updates manifest entry status
- [x] 4.7 Seed `tracker/manifest.yaml` with features from SPEC.md Section 9.1
- [x] 4.8 Write/tests: monitor_test.go (httptest.Server — 200/304/403 scenarios), analyzer_test.go (table-driven classification), manifest_test.go (atomic write concurrency)
